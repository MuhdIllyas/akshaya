...
const ServiceManagementSuperAdmin = ({ currentStaff }) => {
  // State variables
  ...
  const [newServiceRequiresWallet, setNewServiceRequiresWallet] = useState(false);
  const [editServiceRequiresWallet, setEditServiceRequiresWallet] = useState(false);
  ...
  const handleDeleteSubcategory = async (serviceId, subId) => {
    try {
      await deleteSubcategory(serviceId, subId);
      const servicesResponse = await getServices(debouncedSearchQuery);
      setServices(Array.isArray(servicesResponse.data) ? servicesResponse.data : []);
  ...
  const openEditModal = (service) => {
    ...
    setEditServiceRequiresWallet(service.requires_wallet || false);
  ...
  const handleAddService = async (e) => {
    ...
    try {
      ...
      setNewServiceRequiresWallet(false);
  ...
  const handleEditService = async (e) => {
    ...
    try {
      ...
      setEditServiceRequiresWallet(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-indigo-50 p-4 sm:p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-gray-100">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Superadmin Service Management</h1>
            <p className="text-gray-600 mt-1">Manage government services available to all centres</p>
          </div>
          <motion.button
        ...
        {!loading && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
            {[
              ...
              { label: "Active Services", value: services.filter((s) => s.status === "active").length, icon: "✅", bgColor: "from-emerald-100 to-emerald-50", borderColor: "border-emerald-200" },
              { label: "Sub-Categories", value: services.reduce((sum, service) => sum + (service.subcategories?.length || 0), 0), icon: "🗂️", bgColor: "from-amber-100 to-amber-50", borderColor: "border-amber-200" },
            ].map((stat, index) => (
              <motion.div
                key={index}
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm mt-3">{highlightMatch(service.description)}</p>
                    </div>
                  </div>
                    ...
                    <div className="col-span-2 border border-gray-100 bg-blue-50 rounded-xl p-4">
                      <div className="flex items-center">
                        ...
                        <div className="ml-2">
                          ...
                          <p className="font-bold text-gray-900">{service.has_expiry ? "Yes" : "No"}</p>
                        </div>
                      </div>
                    </div>
                    <div className="col-span-2 border border-gray-100 bg-indigo-50 rounded-xl p-4">
                      <div className="flex items-center">
                        <WalletIcon />
                        <div className="ml-2">
                          <p className="text-xs text-gray-600">Wallet Integration</p>
                          <p className="font-bold text-gray-900">{service.requires_wallet ? "Required" : "Not Required"}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          {(service.subcategories && service.subcategories.length > 0) ? (
                            <div className="max-h-80 overflow-y-auto pr-2">
                              {service.subcategories.map((subCat) => {
                                const isMatch = searchQuery && subCat.name?.toLowerCase().includes(searchQuery.toLowerCase());
                                return (
                                  <div
                                    key={subCat.id}
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="text-left">
                                        <p className="font-medium text-gray-900">{highlightMatch(subCat.name)}</p>
                                      ...
                                      <ChevronIcon open={expandedSubCategory === subCat.id} className="h-5 w-5 text-gray-500" />
                                    </button>
        ...
        {!loading && services.length === 0 && (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">{searchQuery ? "No services found" : "No services available"}</h3>
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              {searchQuery ? "Your search didn't match any services. Try different keywords." : "Get started by adding your first service."}
            </p>
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.98 }}
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-gray-500 ml-8">
                        Check this if the service requires a wallet for transactions
                      </p>
                    </div>
                    <div>
                      <label className="flex items-center">
                        <input
                          ...
                          className="rounded text-indigo-600 focus:ring-indigo-500 h-5 w-5"
                        />
                        <span className="ml-3 text-sm text-gray-700 font-medium">This service has an expiry date</span>
                      </label>
                      <p className="mt-1 text-xs text-gray-500 ml-8">
                    ...
                    <div>
                      <label className="flex items-center">
                        <input
                          type="checkbox"
                          name="requiresWallet"
                          checked={editServiceRequiresWallet}
                          onChange={(e) => setEditServiceRequiresWallet(e.target.checked)}
                          className="rounded text-indigo-600 focus:ring-indigo-500 h-5 w-5"
                        />
                        <span className="ml-3 text-sm text-gray-700 font-medium">Requires Wallet Integration</span>
                      </label>
...
export default ServiceManagementSuperAdmin;