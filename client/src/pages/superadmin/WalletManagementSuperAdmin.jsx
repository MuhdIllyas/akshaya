...
const WalletManagementSuperAdmin = () => {
  ...
  // Wallets Opening and Closing Balance Calculations
  const getBalances = (walletId) => {
    const today = new Date().toISOString().split('T')[0];
    const todayTransactions = transactions.filter(
      tx => tx.wallet_id === walletId && tx.created_at.startsWith(today)
    );
    
    // Calculate net change for today
    const netChange = todayTransactions.reduce((sum, tx) => {
      if (tx.type === 'credit') return sum + tx.amount;
      if (tx.type === 'debit') return sum - tx.amount;
      return sum;
    }, 0);

    const currentBalance = wallets.find(w => w.id === walletId)?.balance || 0;
    const openingBalance = currentBalance - netChange;

    return { openingBalance, closingBalance: currentBalance };
  };
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3">
            {filteredWallets
              ...
              .map((wallet) => {
                ...
                const { openingBalance, closingBalance } = getBalances(wallet.id);
                        ...
                        <div className="border border-gray-100 bg-gray-50 rounded-xl p-4">
                          <div className="flex items-center">
                            ...
                            <div className="ml-2">
                              ...
                              <p className="font-bold text-gray-900 text-lg">{formatAmount(openingBalance)}</p>
                            </div>
                          </div>
                        </div>
                        <div className="border border-gray-100 bg-gray-50 rounded-xl p-4">
                          <div className="flex items-center">
                            ...
                            <div className="ml-2">
                              ...
                              <p className="font-bold text-gray-900 text-lg">{formatAmount(closingBalance)}</p>
                            </div>
                          </div>
                        </div>
                      </div>
